...
@section('title')
    {{__('adminmodule::admin.updateAdmin')}}
@endsection
                ...
                <div class="page-title">
                    ...
                    <div class="crumbs">
                        <ul id="breadcrumbs" class="breadcrumb">
                            ...
                            <li><a href="#">{{__('adminmodule::admin.updateAdmin')}}</a></li>
                        ...
                        <div class="widget-header">
                            <div class="row">
                                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                    <h4>{{__('adminmodule::admin.updateAdmin')}}</h4>
                                </div>
                            </div>
                        </div>

                        <div class="widget-content widget-content-area">
                            <form action="{{url('admin/admins/'.$data->id)}}" method="POST" class="needs-validation"
                                  novalidate>
                                @csrf
                                {{ method_field('PUT') }}
                                                    ...
                                                    @foreach($phone_codes as $code)
                                                        <option
                                                            value="{{$code->id}}" {{ old('phone_code_id') == $code->id
                                                                            ? 'selected' : ($data->phone_code_id == $code->id
                                                ...
                                                <div id="select" name="role" class="form-group mb-4">
                                                    <select name="role" class="custom-select" required="">
                                                        ...
                                                        @foreach($roles as $role)
                                                            <option
                                                                {{($data->roles()->first()) ?(($role->name==$data->roles()->first()->name)?'selected':''):''}} value="{{$role->name}}">{{$role->name}}</option>
                                                        @endforeach
                                                    </select>
                                    ...
                                    <div class="col-md-4 col-xs-12">
                                        <div class="form-row">
                                            <div class="col-md-12 col-xs-12">
                                                    ...
                                                    <div class="widget-content ">
                                                        ...
                                                        <select id="orders_zones" name="orders_zones[]"
                                                            ...
                                                            @foreach($zones as $key)
                                                                <option
                                                                    {{(in_array($key->id,$selectedZones)?'selected':'')}} value="{{$key->id}}">{{$key->name_ar}}</option>
                                                            @endforeach
                                                ...
                                                <div class="col-md-12 col-xs-12">
                                                    <label class=""
                                                           for="viewed_levels">{{__('adminmodule::admin.statuses')}}</label><br>
                                                    <select id="status_levels" name="status_levels[]"
                                                        ...
                                                        @foreach($status as $key)
                                                            <option
                                                                {{(in_array($key->id,$status_levels)?'selected':'')}} value="{{$key->id}}">{{$key->title}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-md-12 col-xs-12">
                                                    ...
                                                    <input data-validate-func="required" data-validate-arg="6"
                                                           id="order_type"
                                                           name="order_type" type="radio"
                                                           value="0" {{ ($data->order_type==0)?'checked':''}}> {{__('adminmodule::admin.customers')}}
                                                    <input data-validate-func="required" data-validate-arg="6"
                                                           id="order_type"
                                                           name="order_type" type="radio"
                                                           value="1" {{ ($data->order_type==1)?'checked':''}}> {{__('adminmodule::admin.merchants')}}
                                                    <input data-validate-func="required" data-validate-arg="6"
                                                           id="order_type"
                                                           name="order_type" type="radio"
                                                           value="2" {{ ($data->order_type==2)?'checked':''}}>
                                ...
                                <button class="btn btn-gradient-danger mb-4"
                                        type="submit">{{__('adminmodule::admin.update')}}</button>
                            </form>
                        </div>
                    </div>
                </div>
