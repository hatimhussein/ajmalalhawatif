...
@section('content')
    ...
    <!-- Breadcrumbs -->
    @include('fronthomemodule::content.breadCrumbs',['pages'=>[$word]])
        ...
        <div class="main container">
            @if(count($products) > 0)
                <div class="row">
                                    ...
                                    <div class="block-content">
                                                        ...
                                                        @foreach($scategories as $category)
                                                            <li>
                                                                <span title="Black"><input class="category_id"
                                                                                           name="category_ids[]"
                                                                                           type="checkbox"
                                                                                           value="{{$category->id}}"/> {{$category->name_ar}}</span>

                                                            </li>
                                                        @endforeach
                                                    @else
                                                        @foreach($scategories as $category)
                                                            @if($category->child->count() == 0 && is_null($category->parent_id))
                                                                <li>
                                                                    <span title="Black"><input class="category_id"
                                                                                               name="category_ids[]"
                                                                                               type="checkbox"
                                                            ...
                                                            @foreach($category->child as $child)
                                                                <li>
                                                                    <span title="Black"><input class="category_id"
                                                                                               name="category_ids[]"
                                                                                               type="checkbox"
                                                                                               value="{{$child->id}}"/> {{$child->name_ar}}</span>
                                                ...
                                                <dd id="collapseTwo" class="collapse" aria-labelledby="headingTwo"
                                                    ...
                                                    <ol class="m-filter-css-checkboxes ">
                                                        @foreach($cat_brands as $brand)
                                                            <li>
                                                                <span title="Black"><input class="brands"
                                                                                           name="brands[]"
                                                                                           type="checkbox"
                                                                                           value="{{$brand->id}}"/> {{$brand->name_ar}}</span>
                                        ...
                                        <dl class="narrow-by-list">
                                            ...
                                            <dd id="collapseSix" class="collapse" aria-labelledby="headingSix"
                                                ...
                                                <!-- <li>
                                                      <span  title="Black"><input class="price" name="price"  type="radio" value="0,1000" /> 0 {!! LanguageHelper::nameTranslate(Cookie::get('currency')) !!} - 10000 {!! LanguageHelper::nameTranslate(Cookie::get('currency')) !!}</span>

                                                  </li>
                                                  <li>
                                                      <span  title="Black"><input class="price" name="price"  type="radio" value="10000,20000" /> 10000 {!! LanguageHelper::nameTranslate(Cookie::get('currency')) !!} - 20000 {!! LanguageHelper::nameTranslate(Cookie::get('currency')) !!}</span>
                    ...
                    <input type="hidden" name="word" id="hidden_word" value="{{$word}}"/>
            ...
            @else
                <div>
                    <h2 class="text-center">{{__('productmodule::product.search_no_products')}}</h2>
                </div>
            @endif
        </div>
    </section>
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        function getData(page, sort_type, sort_by) {
            ...
            var word = $("#hidden_word").val();
            pagee = 1;
            $.ajax(
                {
                    url: '?page=' + page + "&sortby=" + sort_by + "&sorttype=" + sort_type + "&options_values=" + options_values + "&category_ids=" + category_ids + "&price=" + price + "&brands=" + brands + "&word=" + word,
                    type: "get",
                    datatype: "html"
                }).done(function (data) {
        ...
        function infiniteLoadMore(page) {
            ...
            var word = $("#hidden_word").val();
            $.ajax({
                url: '?page=' + page + "&sortby=" + column_name + "&sorttype=" + sort_type + "&options_values=" + options_values + "&category_ids=" + category_ids + "&price=" + price + "&brands=" + brands + "&word=" + word + "&typereturn=5",
                datatype: "html",
                type: "get",
                beforeSend: function () {
                    $('.auto-load').show();
