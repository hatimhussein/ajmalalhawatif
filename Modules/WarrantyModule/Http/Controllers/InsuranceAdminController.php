    ...
    public function insuranceServer(Request $request)
    {
        ...
        if(!empty($Search))
        {
            $insurances->where(function ($qu) use ($Search){
               $qu->where('id',$Search)->orWhere('user_name','like','%'.$Search.'%')->orWhereHas('merchant',function ($q) use($Search){
                   $q->where('company_name','like','%'.$Search.'%')->orWhere('account_number','like','%'.$Search.'%');
                ...
                if($Search == __('warrantymodule::insurance.activated')){
                    $que->where('status',1);
                }
                if($Search ==__('warrantymodule::insurance.pending')){
                    $que->where('status',0);
                }
                if($Search == __('warrantymodule::insurance.rejected')){
                    $que->where('status',2);
                }
                if($Search == __('warrantymodule::insurance.closed')){
        ...
        foreach ($insurances as $insurance)
        {
            ...
            if($insurance->isClosed()){
                $status = ' <span class="badge badge-dark">'.__('warrantymodule::insurance.closed').'</span>';
            }else
            {
                if($insurance->status == 1){
                    $status = ' <span class="badge badge-success">'.__('warrantymodule::insurance.activated').'</span>';
                }elseif($insurance->status == 0)
                {
                    $status = '<span class="badge badge-info">'.__('warrantymodule::insurance.pending').'</span>';
                }elseif($insurance->status == 3)
                    if(is_null($insurance->seen_at))
                {
                    $status = '<span style="margin-bottom: 10px;" class="badge badge-warning">'.__('warrantymodule::warranty.in_progress').'</span> '.'<span class="badge badge-info">'.__('warrantymodule::insurance.replay_done').'</span>'. '<span class="badge badge-primary">'.__($updated_at).'</span>';
                }
                    else
                    {
                        $status = '<span style="margin-bottom: 10px;" class="badge badge-warning">'.__('warrantymodule::warranty.in_progress').'</span>';

                    }
                else
                {
                    $status = ' <span class="badge badge-danger">'.__('warrantymodule::insurance.rejected').'</span>';
                }
            ...
            $action = '<ul class="table-controls">
                       <li><a href="'.route('insurance.edit', $insurance->id).'"
                             data-toggle="tooltip" data-placement="top" title="Edit">
                               <i class="flaticon-edit  bg-success p-1 text-white"></i>
                             </a></li>
                       <li>
                       <form class="inline" action="'.route('insurance.destroy', $insurance->id) .'" method="POST">
            ...
            $responce[]=[$id,$merchant,$merchant_account,$user_name,$phone,$email,$dummy_text_1,$dummy_text_2,$dummy_text_3,$attachments,$usage_date, $created_at,$replied_at,$status,$expire_date,$adminName,$action];
