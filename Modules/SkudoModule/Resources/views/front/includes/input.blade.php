@if($input->key == 'phone')
    <div class="row skudo-phone-row" style="display: flex;">
        <div class="col-md-8 col-xs-8">
            <label for="phone"
                   class="required">{{__('skudomodule::insurance.phone')}}
...
@else
    <label for="{{ $input->key }}">{{__('skudomodule::'.($localeFile ?? 'warranty').'.'.$input->key)}}
        @if($input->value_en)<em class="required">*</em>@endif
    </label>
    <div class="input-box">
        <input type="{{ $input->properties['type'] }}" name="{{ $input->key }}"
               {{ ($read_only ?? false) ? 'readonly' : '' }}
               {{ ($disabled ?? false) ? 'readonly' : '' }}
               title="{{__('skudomodule::'.($localeFile ?? 'warranty').'.'.$input->key)}}" id="{{ $input->key }}"
               class="input-text form-control {{ $input->value_en ? 'required-entry' : '' }}"
               value="{{ $value ?? '' }}">
    </div>
@endif
