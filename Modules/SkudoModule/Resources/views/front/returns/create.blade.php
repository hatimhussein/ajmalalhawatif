        ...
        <div class="main container">
            <div class="row">
                ...
                <section class="col-md-9 col-sm-6 wow bounceInUp animated return">
                    <form id="return-form" action="{{route('front.skudo.returns.store')}}" method="POST">
                        @csrf
                        <div id="pick-returns" class="returns-tab">
                            <h3> {{__('skudomodule::returns.choose_products')}} </h3>
                            <div class="corner-buttons">
                                <a href="{{route('front.skudo.returns.index')}}" class="btn btn-default">
                                {{__('skudomodule::returns.cancel')}}
                                </a>
                                <button type="button" role="button" class="btn btn-info tab-switch action-btn"
                                        data-target="#pick-address" style="display: none">
                                    {{__('skudomodule::returns.choose_address')}}
                                </button>
                            </div>
                                    ...
                                    <div class="col-lg-5">
                                        <div class="customer-name row">
                                            <div class="field name-firstname col-md-12">
                                                <h5> {{__('skudomodule::returns.order_id')}} {{$product->order_id}}</h5>
                                                <label> {{__('skudomodule::returns.return_reason_lbl')}} </label>
                                                <div class="input-box">
                                                    <select name="reasons[{{$product->id}}]"
                                                            class="input-text form-control required-entry">
                                                        <option selected disabled>
                                                            {{__('skudomodule::returns.choose_reason')}}
                                                        </option>
                ...
                'statusCode': {
                    200: function (response) {
                        if (response.code === 201) {
                            ...
                            setTimeout(function () {
                                window.location = "{{route('front.skudo.returns.index')}}";
                            }, 3000);
                        }
                    },
