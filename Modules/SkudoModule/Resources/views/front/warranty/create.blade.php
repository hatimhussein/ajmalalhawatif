                        ...
                        <div class="my-account">
                            ...
                            @if($type == 'sms')
                                <div class="alert alert-success">
                                    <strong>{{ __('skudomodule::warranty.insurance_phone_inquiry') }}</strong>
                                    <p>{{ __('skudomodule::warranty.enter_insurance_phone') }}</p>
                                    <small class="text-muted">{{ __('skudomodule::warranty.insurance_phone_help') }}</small>
                                </div>
                            @endif
                                                ...
                                                <div class="col-md-6">
                                                    @if($inputs->contains('key', 'warranty_number'))
                                                        ...
                                                        <div class="form-group">
                                                            <label for="warranty_number">{{ __('skudomodule::warranty.phone') }}
                                                                @if($warrantyInput->value_en)<em class="required">*</em>@endif
                                                            </label>
                                                            <div class="input-box">
                                                                <div class="input-group" id="warranty_number_group" style="direction: rtl;">
                                                                    <input type="text" name="phone_search" id="warranty_number" 
                                                                        title="{{ __('skudomodule::warranty.phone') }}"
                                                                        class="input-text form-control {{ $warrantyInput->value_en ? 'required-entry' : '' }}" 
                                                                        placeholder="{{ __('skudomodule::warranty.insurance_phone_placeholder') }}"
                                                                        style="border-top-left-radius: 0; border-bottom-left-radius: 0; border-left: 0;">
                                                                    <div class="input-group-append">
                                                                        <button type="button" id="search_warranty_btn" class="btn btn-primary" 
                                                                            ...
                                                                            {{ __('skudomodule::warranty.insurance_phone_search_button') }}
                                                                        </button>
                                                                    </div>
                                                                </div>
                                                                <input type="hidden" name="warranty_number" id="selected_warranty_number" value="">
                                                                <small class="form-text text-muted" style="font-size: 11px; font-weight: bold; color: black;">{{ __('skudomodule::warranty.insurance_phone_help') }}</small>
                                                            </div>
                                                        </div>
                                                    @endif
    ...
    {{--                                    </div>--}}
                            ...
                            {{-- Modal: select insurance when multiple results found --}}
                            <div class="modal fade" id="insuranceSelectModal" tabindex="-1" role="dialog" aria-labelledby="insuranceSelectModalTitle" aria-hidden="true">
                                <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="insuranceSelectModalTitle">{{ __('skudomodule::warranty.select_insurance_modal_title') }}</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
                                        <div class="modal-body">
                                            <p class="text-muted mb-3">{{ __('skudomodule::warranty.select_insurance_modal_hint') }}</p>
                                            <div class="table-responsive">
                                                <table class="table table-bordered table-hover">
                                                    <thead>
                                                    <tr>
                                                        <th>{{ __('skudomodule::warranty.select_insurance_table_id') }}</th>
                                                        <th>{{ __('skudomodule::warranty.select_insurance_table_package_serial') }}</th>
                                                        <th>{{ __('skudomodule::warranty.select_insurance_table_device_serial') }}</th>
                                                        <th>{{ __('skudomodule::warranty.select_insurance_table_created_at') }}</th>
                                                        <th>{{ __('skudomodule::insurance.status') }}</th>
                                                        <th>{{ __('skudomodule::warranty.select_insurance_table_action') }}</th>
                                                    </tr>
                                                    </thead>
                                                    <tbody id="insuranceSelectModalBody"></tbody>
                                                </table>
                                            </div>
                                        </div>
                                    </div>
    ...
    @section('js')
        ...
        @if ($type == 'sms')
            <script type="text/javascript">
                ...
                const phoneUrl = '{!! route('skudo.warranty.insurances_by_phone') !!}';
                let isLoading = false;
                window._skudoInsuranceById = {};
                
                // نصوص الحالات المترجمة
                const statusTexts = {
                    closed: '{{__('skudomodule::insurance.closed')}}',
                    activated: '{{__('skudomodule::insurance.activated')}}',
                    in_progress: '{{__('skudomodule::warranty.in_progress')}}',
                    pending: '{{__('skudomodule::insurance.pending')}}',
                    rejected: '{{__('skudomodule::insurance.rejected')}}'
                };
                
                // إخفاء البيانات عند تعديل الحقل
                $('#warranty_number').on('input', function () {
                    const phone = $(this).val().trim();
                    $('#selected_warranty_number').val('');
                ...
                $('#search_warranty_btn').on('click', function() {
                    const phone = $('#warranty_number').val().trim();
                    
                    // التحقق من أن الرقم ليس فارغاً
                    if (!phone) {
                        toastr["warning"]("{{ __('skudomodule::warranty.enter_phone_first') }}");
                        return;
                    }
                ...
                function performSearch(phone) {
                    ...
                    $('#search_warranty_btn').prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> {{ __('skudomodule::warranty.insurance_phone_loading') }}');
                    ...
                    $.get(phoneUrl, { phone })
                        .done(response => {
                            ...
                            if (!insurances.length) {
                                fillInsuranceInputs();
                                $('#selected_warranty_number').val('');
                                toastr["error"]("{{ __('skudomodule::warranty.no_insurance_found_for_phone', ['phone' => ':phone']) }}".replace(':phone', phone));
                                return;
                            }

                            if (insurances.length === 1) {
                                selectInsurance(insurances[0]);
                                return;
                            }

                            // Multiple: show modal list
                            renderInsuranceSelectModal(insurances);
                            $('#insuranceSelectModal').modal('show');
                        })
                        .fail(response => {
                            fillInsuranceInputs();
                            hideLoader();
                            toastr["error"]("{{ __('skudomodule::warranty.insurance_fetch_error') }}");
                        })
                        .always(() => {
                            isLoading = false;
                            // إعادة تفعيل الزر
                            $('#search_warranty_btn').prop('disabled', false).html('{{ __('skudomodule::warranty.insurance_phone_search_button') }}');
                        });
                }

                function renderInsuranceSelectModal(insurances) {
                    window._skudoInsuranceById = {};
                    insurances.forEach(i => window._skudoInsuranceById[i.id] = i);

                    const tbody = $('#insuranceSelectModalBody');
                    tbody.html('');
                            ...
                            <tr class="${rowClass}">
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-info" onclick="selectInsuranceById(${ins.id})">
                                        {{ __('skudomodule::warranty.select_insurance_choose') }}
                                    </button>
                                </td>
                            </tr>
                        `);
                ...
                function selectInsuranceById(id) {
                    const insurance = window._skudoInsuranceById?.[id];
                    if (!insurance) return;
                    selectInsurance(insurance);
                    $('#insuranceSelectModal').modal('hide');
                }

                function selectInsurance(insurance) {
                    // set hidden warranty_number (insurance id) for submission
                    $('#selected_warranty_number').val(insurance?.id ?? '');
                ...
                function showLoader() {
                    ...
                    if ($('#warranty-loader').length === 0) {
                        $('body').append('<div id="warranty-loader" class="warranty-loader-overlay"><div class="warranty-loader-content"><div class="warranty-spinner"></div><p>{{ __('skudomodule::warranty.insurance_phone_loading') }}</p></div></div>');
                    }
