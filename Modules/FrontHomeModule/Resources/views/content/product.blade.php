                ...
                <div class="item-content">
                    <div class="ratings">
                        <div class="rating-box">
                            @if($product->reviews->where('is_shown',1)->count() > 0)
                                <div
                                    style="width:{{$product->reviews->where('is_shown',1)->sum('stars') / $product->reviews->where('is_shown',1)->count()}}%"
                                    class="rating"></div>
                            @else
                ...
                @if(trim($product->type)=="simple")
                    @if ($product->product_quantity > 0)
                        @if($discount)
                            <button data-product_id="{{$product->id}}" data-cart_type="from_home"
                                    data-product_photo="{{$product->product_photo}}"
                                    data-product_type="{{$product->type}}"
                                    data-product_name="{!! LanguageHelper::nameTranslate($product) !!}"
                                    data-product_price="{!! ProductHelper::calDiscountAmountWithoutCurrency($product->tax_free_price,$discount) !!}"
                    ...
                    @else
                        <a href="{{url('product-details/'.$product->id)}}" class="button btn-cart">
                            <span>{{__('productmodule::product.out_stock')}}</span>
                        </a>
                    @endif
                @else
                    <a href="{{url('product-details/'.$product->id)}}" class="button btn-cart">
