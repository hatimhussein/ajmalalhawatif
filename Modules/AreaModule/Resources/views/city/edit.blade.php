...
@section('title')
    {{__('areamodule::area.update_city')}}
@endsection
...
@section('content')
  <div id="content" class="main-content">
      <div class="container">
          ...
          <div class="row">
            <form  action="{{url('admin/city/'.$city->id)}}" style="width:100%"  method="POST"  data-role="validator" data-on-before-submit="no_submit" data-on-error-input="notifyOnErrorInput" data-show-error-hint="false" novalidate="novalidate" enctype="multipart/form-data">
              @csrf
              {{ method_field('PUT') }}
                        ...
                        <div class="widget-header">
                            <div class="row">
                                <div class="col-xl-9 col-md-9 col-sm-9 col-9">
                                    <h4>{{__('areamodule::area.update_city')}}</h4>
                                  </div>
                                  ...
                                  <div class="col-md-6 col-9 ">
                                      <div  class="statbox widget box box-shadow">
                                          <label  >{{__('areamodule::area.country')}}</label>
                                              ...
                                              <select id="country_id" name="country_id" class="disabled-results form-control custom-select" >
                                                ...
                                                @foreach($countries as $country)
                                                  <option {{(($city->country!=null) && $country->id == $city->country->id)?'selected':''}}  value="{{$country->id}}">{{$country->name_ar}}</option>
                                                @endforeach
                                              </select>
                                          ...
                                          <label  >{{__('areamodule::area.government')}}</label>
                                              ...
                                              <select id="government_id" name="government_id" class="disabled-results form-control custom-select" >
                                                ...
                                                @foreach($governments as $government)
                                                  <option {{(($city->government!=null) && $government->id == $city->government->id)?'selected':''}}  value="{{$government->id}}">{{$government->name_ar}}</option>
                                                @endforeach
                                        ...
                                        <div class="input-control required col-md-12 mb-4 required pl-0">
                                          <label class="col-md-12 mt-4 pl-0">{{__('areamodule::area.name_ar')}}</label>

                                            <input name="name_ar" value="{{$city->name_ar}}" class="form-control" data-validate-func="required" data-validate-arg="6"  data-validate-hint="{{__('areamodule::area.rname_ar')}}" placeholder="{{__('areamodule::area.name_ar')}}" autocomplete="off">
                                            @if ($errors->has('name_ar'))
                                              @include('commonmodule::includes.error',['errors'=>$errors->toArray(),'filed'=>'name_ar'])
                                            @endif
                                        </div>
                                        <label class="col-md-12 mt-4 pl-0">{{__('areamodule::area.name_en')}}</label>

                                        <div class="input-control required col-md-12 mb-4 pl-0">
                                            <input name="name_en" value="{{$city->name_en}}" class="form-control" data-validate-func="required" data-validate-arg="6"  data-validate-hint="{{__('areamodule::area.rname_en')}}" placeholder="{{__('areamodule::area.name_en')}}" autocomplete="off">
                                            @if ($errors->has('name_en'))
                                              @include('commonmodule::includes.error',['errors'=>$errors->toArray(),'filed'=>'name_en'])
                                            @endif
                                        </div>

                                              <label class="pl-0 col-md-12">{{__('areamodule::area.shipping_price')}}</label>
                                              <div class="input-control required col-md-12 mb-4 pl-0">
                                                  <input  name="shipping_price" value="{{ $city->shipping_price }}" class="form-control" data-validate-func="required" data-validate-arg="6"  data-validate-hint="{{__('areamodule::area.rshipping_price')}}" placeholder="{{__('areamodule::area.shipping_price')}}" autocomplete="off">
                                                  @if ($errors->has('shipping_price'))
                                                    @include('commonmodule::includes.error',['errors'=>$errors->toArray(),'filed'=>'shipping_price'])
                                                  @endif
