...
@section('title')
    ...
    {{__('areamodule::area.governments')}}
@endsection
        ...
        <div class="container">
            <div class="page-header">
                <div class="page-title">
                    <h3>{{__('areamodule::area.governments')}}</h3>

                </div>

                @can('add_government')
                    <div class="page-title" style="float:right">
                        <a href="{{url('admin/government/create')}}"
                           class="mt-4 btn btn-button-16"> {{__('areamodule::area.add_new_government')}} </a>
                        <a class="mt-4 btn btn-button-16 mr-2"
                           href="{{url('admin/downloadgovernment')}}"> {{__('areamodule::area.download')}}  </a>
                        ...
                        <div class="widget-header">
                            <div class="row">
                                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                    <h4>{{__('areamodule::area.governments')}} </h4>
                                </div>
                            </div>
                            @can('delete_government')
                                <div class="row">
                                    <div class="col-md-6">
                                        <form class="bulk-form" action="{{ route('government.bulk') }}" method="post">
                                            @csrf
                                        ...
                                        <tr class="text-center">
                                            ...
                                            <td>
                                                <input type="checkbox" class="table-select"
                                                       name="ids[]" value="{{$government->id}}">
                                            </td>
                                            <td>{{$government->name_ar}}</td>
                                            <td>{{$government->name_ar}}</td>
                                            <td>{{($government->country)?$government->country->name_ar:'-'}}</td>

                                            <td>
                                                <ul class="table-controls">


                                                    @can('update_government')
                                                        <li>
                                                            <a href="{{url('admin/government/'.$government->id.'/edit')}}"
                                                    ...
                                                    @can('delete_government')
                                                        <li>
                                                            <form class="inline"
                                                                  action="{{url('admin/government/' . $government->id)}}"
                                                                  method="POST">
                                                                {{ method_field('DELETE') }} {!! csrf_field() !!}
                                                                <button class="unst" title="Delete" type="submit"
                                                                        onclick="return confirm('{{__("areamodule::area.delete_government")}}')"
                                                                        type="button"
                                                                >
    ...
    <div class="modal fade" id="uploadModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{url('admin/uploadgovernment')}}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row">

                            <input type="file" name="government" class="col-lg-6" required>
                        </div>
                    </div>
