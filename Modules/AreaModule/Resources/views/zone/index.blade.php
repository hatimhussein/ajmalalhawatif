        ...
        <div class="container">
            <div class="page-header">
                <div class="page-title">
                    <h3>{{__('areamodule::area.zones')}}  </h3>

                </div>

                @can('add_zone')
                    <div class="page-title" style="float:right">
                        <a href="{{url('admin/zone/create')}}"
                           class="mt-4 btn btn-button-16"> {{__('areamodule::area.add_new_zone')}} </a>
                        <a class="mt-4 btn btn-button-16 mr-2"
                           href="{{url('admin/downloadzone')}}"> {{__('areamodule::area.download')}}  </a>
                        ...
                        <div class="widget-header">
                            <div class="row">
                                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                    <h4>{{__('areamodule::area.zones')}}</h4>
                                </div>
                            </div>
                            @can('delete_zone')
                                <div class="row">
                                    <div class="col-md-6">
                                        <form class="bulk-form" action="{{ route('zone.bulk') }}" method="post">
                                            @csrf
                                        ...
                                        <tr class="text-center">
                                            ...
                                            <td>
                                                <input type="checkbox" class="table-select"
                                                       name="ids[]" value="{{$zone->id}}">
                                            </td>
                                            <td>{{$zone->name_ar}}</td>
                                            <td>{{$zone->name_en}}</td>
                                            <td>{{($zone->city)?$zone->city->name_ar:''}}</td>
                                            <td>{{($zone->government)?$zone->government->name_ar:''}}</td>
                                            <td>{{($zone->country)?$zone->country->name_ar:''}}</td>
                                            ...
                                            <td>
                                                <ul class="table-controls">
                                                    ...
                                                    @can('update_zone')
                                                        <li><a href="{{url('admin/zone/'.$zone->id.'/edit')}}"
                                                    ...
                                                    @can('delete_zone')
                                                        <li>
                                                            <form class="inline"
                                                                  action="{{url('admin/zone/' . $zone->id)}}"
                                                                  method="POST">
                                                                {{ method_field('DELETE') }} {!! csrf_field() !!}
                                                                <button class="unst" title="Delete" type="submit"
                                                                        onclick="return confirm('{{__("areamodule::area.delete_zone")}}')"
                                                                        type="button"
                                                                >
    ...
    <div class="modal fade" id="uploadModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{url('admin/uploadzone')}}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row">

                            <input type="file" name="zone" class="col-lg-6" required>
                        </div>
                    </div>
